...
import { mockUsers, mockWorkspaces } from "@/mock";
...
export default function AdminDashboard() {
  ...
  const roleDistribution = useMemo(() => {
    const counts = mockUsers.reduce<Record<string, number>>((acc, u) => {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-5">
        <StatCard label="Total Users" value={mockUsers.length} icon={Users} accent="bg-primary-50 text-primary-600 dark:bg-primary-500/10 dark:text-primary-400" delay={0} />
        ...
        <StatCard label="Workspaces" value={mockWorkspaces.length} icon={Trello} accent="bg-warning-50 text-warning-600 dark:bg-warning-500/10 dark:text-warning-500" delay={0.09} />
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-2">
            {mockWorkspaces.map((ws) => {
              const boardCount = boards.filter((b) => b.workspaceId === ws.id).length;
              return (
                <div key={ws.id} className="flex items-center justify-between rounded-lg border border-border px-3 py-2.5">
                  <div className="flex items-center gap-3">
                    <span className="h-8 w-8 rounded-lg" style={{ backgroundColor: ws.logoColor }} />
                    <div>
                      <p className="text-sm font-semibold">{ws.name}</p>
                      <p className="text-xs text-muted-foreground">/{ws.slug}</p>
                    </div>
                  </div>
                  <Badge variant="secondary">{boardCount} boards</Badge>
                </div>
              );
            })}
          </CardContent>
